...
function Pattern(pattern) {
    ...
    function format(loggerName, logLevel, logText, args) {
        ...
        var box = 0;
        while (box != -1 && args.length > 0) {
            box = logText.indexOf('%b');
            if (box != -1) {
                var arg = args.shift();
                logText = logText.substring(0, box) + '[' + arg + ']' + logText.substring(box + 2);
            }
        }
...
function reconfigureDefaultConsole() {
    // save the real console function since the console appender uses them.
